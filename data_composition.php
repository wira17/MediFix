...
// ── Serve log ─────────────────────────────────────────────────────
if (isset($_GET['_log']) && $_GET['_log'] === 'composition') {
    header('Content-Type: text/plain; charset=utf-8');
    $logFile = __DIR__ . '/logs/composition_' . date('Y-m') . '.log';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $action = $_POST['action'] ?? '';
    if (in_array($action, ['kirim_composition', 'kirim_semua_composition'])) {
        include __DIR__ . '/api/kirim_composition.php';
        exit;
    }
}
...
try {
    ...
    if ($filter_status === 'terkirim') {
        $statusWhere = " AND sd.id_diet IS NOT NULL AND sd.id_diet != ''";
    } elseif ($filter_status === 'pending') {
        $statusWhere = " AND (sd.id_diet IS NULL OR sd.id_diet = '')";
    }

    $cariWhere  = '';
    $cariParams = [];
    if (!empty($cari)) {
        $cariWhere  = " AND (rp.no_rawat LIKE ? OR rp.no_rkm_medis LIKE ? OR p.nm_pasien LIKE ? OR cag.instruksi LIKE ?)";
    ...
    $baseSQL = "
        ...
        LEFT JOIN satu_sehat_encounter se ON se.no_rawat  = cag.no_rawat
    ...
    $stmtData = $pdo_simrs->prepare("
        SELECT
            ...
            IFNULL(sd.id_diet,'') AS id_diet
        $baseSQL
        ORDER BY rp.tgl_registrasi DESC, cag.tanggal DESC
    ...
    $stmtStats = $pdo_simrs->prepare("
        SELECT
            COUNT(*) AS total,
            SUM(CASE WHEN sd.id_diet IS NOT NULL AND sd.id_diet != '' THEN 1 ELSE 0 END) AS terkirim,
            SUM(CASE WHEN sd.id_diet IS NULL OR sd.id_diet = '' THEN 1 ELSE 0 END) AS pending
        FROM catatan_adime_gizi cag
        JOIN reg_periksa rp ON rp.no_rawat = cag.no_rawat
        LEFT JOIN satu_sehat_diet sd ON sd.no_rawat = cag.no_rawat AND sd.tanggal = cag.tanggal
        WHERE rp.tgl_registrasi BETWEEN ? AND ?
...
$page_title     = 'Diet/Gizi (Composition) — Satu Sehat';

$extra_css = '
.btn-send{width:32px;height:32px;border-radius:6px;padding:0;display:inline-flex;align-items:center;justify-content:center;border:1px solid transparent;cursor:pointer;transition:all .25s;font-size:12px;background:#27ae60;border-color:#219a52;color:#fff}
.btn-send:hover{background:#219a52;transform:scale(1.12);box-shadow:0 3px 10px rgba(39,174,96,.45)}
...
.norawat-lbl{font-weight:700;color:#27ae60;font-size:13px;font-family:"Courier New",monospace}
...
.btn-kirim-semua{background:linear-gradient(135deg,#27ae60,#219a52);border:none;color:#fff;padding:6px 14px;border-radius:5px;font-size:13px;font-weight:600;cursor:pointer;display:inline-flex;align-items:center;gap:6px;transition:all .25s}
.btn-kirim-semua:hover{transform:translateY(-2px);box-shadow:0 4px 14px rgba(39,174,96,.4);color:#fff}
...
.prog-fill-comp{height:100%;border-radius:4px;background:linear-gradient(90deg,#27ae60,#2ecc71);transition:width .6s}
.prog-pct{width:38px;text-align:right;font-weight:700;color:#555;flex-shrink:0}
.tbl-comp thead tr th{background:#27ae60;color:#fff!important;white-space:nowrap;font-size:12px}
...
.btn-period:hover{background:#27ae60;color:#fff;border-color:#27ae60}
.instruksi-lbl{font-size:11.5px;color:#2c3e50;line-height:1.5;max-width:220px;word-break:break-word}
.adime-tag{background:#e8f5e9;color:#2e7d32;border:1px solid #c8e6c9;padding:1px 5px;border-radius:4px;font-size:9px;font-weight:700;margin-right:3px}
...
function kirimSatu(noRawat,tanggal,btnEl) {
    ...
    fetch(window.location.href,{method:"POST",headers:{"Content-Type":"application/x-www-form-urlencoded"},
        body:new URLSearchParams({action:"kirim_composition",no_rawat:noRawat,tanggal:tanggal})})
    .then(r=>r.json()).then(resp=>{
        btnEl.classList.remove("spin");
        if(resp.status==="ok"){
            btnEl.classList.add("sent");btnEl.innerHTML=`<i class="fa fa-check"></i>`;
            showToast("✓ Composition berhasil dikirim!","success");
            const row=btnEl.closest("tr");
            if(row){
                row.classList.add("row-sent");
                const badge=row.querySelector(".badge-comp-status");
                if(badge){badge.className="badge-comp-status badge-sent";badge.innerHTML=`<i class="fa fa-check-circle"></i> Terkirim`;}
                const idCell=row.querySelector(".id-comp-cell");
                if(idCell&&resp.id_diet){
                    const short=resp.id_diet.length>36?resp.id_diet.substring(0,36)+'…':resp.id_diet;
                    idCell.innerHTML=`<span class="id-cell" title="${resp.id_diet}">${short}</span>
                        <button class="btn-copy" onclick="copyText('${resp.id_diet}')"><i class="fa fa-copy"></i></button>`;
                }
            }
...
function kirimSemua() {
    ...
    if(!confirm(`Kirim ${sisa} data Diet/Gizi yang belum terkirim?\nPeriode: ${dari===sampai?dari:dari+" s/d "+sampai}`))return;
    ...
    fetch(window.location.href,{method:"POST",headers:{"Content-Type":"application/x-www-form-urlencoded"},
        body:new URLSearchParams({action:"kirim_semua_composition",tgl_dari:dari,tgl_sampai:sampai})})
    .then(r=>r.json()).then(resp=>{
        ...
        <div class="box-body">
          <form method="GET" class="form-inline" style="display:flex;flex-wrap:wrap;gap:10px;align-items:flex-end;">
            ...
            <div class="form-group">
              ...
              <input type="text" name="cari" class="form-control" placeholder="Pasien / No. Rawat / Instruksi Diet…"
            ...
            <a href="data_composition.php" class="btn btn-default"><i class="fa fa-refresh"></i> Reset</a>
          </form>
        </div>
      </div>

      <!-- Tabel -->
      <div class="box box-primary" style="border-top-color:#27ae60;">
        <div class="box-header with-border">
          <h3 class="box-title">
            <i class="fa fa-leaf"></i> Data Diet / Gizi
            <span class="badge" style="background:#27ae60;"><?= number_format($total) ?></span>
          </h3>
        ...
        <div class="info-bar-stats">
          <div class="ibs-item"><i class="fa fa-database" style="color:#27ae60;"></i> Total: <span class="ibs-val" style="color:#27ae60;"><?= number_format($st_total) ?></span></div>
          ...
          <span class="ibs-sep">|</span>
          <div class="ibs-item" style="font-size:11px;color:#777;"><i class="fa fa-info-circle"></i> Data dari <strong>catatan_adime_gizi</strong> yang ada instruksi diet</div>
        </div>

        <div class="prog-dual">
          <div class="prog-row">
            <span class="prog-label"><i class="fa fa-leaf"></i> Diet/Gizi</span>
            <div class="prog-bar"><div class="prog-fill-comp" style="width:<?= $pct ?>%;"></div></div>
        ...
        <div class="box-body" style="padding:0;">
          ...
          <div class="table-responsive">
            <table class="table table-bordered table-striped table-hover tbl-comp" style="margin-bottom:0;font-size:12px;">
              <thead>
                <tr>
                  ...
                  <th width="220">Instruksi Diet (ADIME)</th>
                  <th width="100">Tgl Catatan</th>
                  <th width="130">Dokter/Ahli Gizi</th>
                  <th>ID Composition</th>
                  <th width="110" class="text-center">Status</th>
                </tr>
              </thead>
              <tbody>
              <?php $no = $offset + 1; foreach ($data as $r):
                  $isSent  = !empty($r['id_diet']);
                  $shortId = $isSent ? mb_strimwidth($r['id_diet'], 0, 36, '…') : '';
              ?>
              <tr class="<?= $isSent ? 'row-sent' : '' ?>">
                ...
                <td class="text-center">
                  <button class="btn-send <?= $isSent?'sent':'' ?>"
                          onclick="kirimSatu('<?= addslashes($r['no_rawat']) ?>','<?= htmlspecialchars($r['tanggal']) ?>',this)"
                ...
                <td>
                  <div class="instruksi-lbl">
                    <?php if (!empty($r['instruksi'])): ?>
                      <span class="adime-tag">Diet</span><?= htmlspecialchars(mb_strimwidth($r['instruksi'], 0, 80, '…')) ?>
                    <?php endif; ?>
                  </div>
                  <div style="margin-top:4px;display:flex;flex-wrap:wrap;gap:2px;">
                    <?php if (!empty($r['asesmen'])):    echo '<span class="adime-tag">A</span>'; endif; ?>
                    <?php if (!empty($r['diagnosis'])):  echo '<span class="adime-tag">D</span>'; endif; ?>
                    <?php if (!empty($r['intervensi'])): echo '<span class="adime-tag">I</span>'; endif; ?>
                    <?php if (!empty($r['monitoring'])): echo '<span class="adime-tag">M</span>'; endif; ?>
                    <?php if (!empty($r['evaluasi'])):   echo '<span class="adime-tag">E</span>'; endif; ?>
                  </div>
                </td>

                <td>
                  <div><?= date('d/m/Y', strtotime($r['tanggal'])) ?></div>
                  <div style="font-size:10.5px;color:#aaa;"><?= date('H:i', strtotime($r['tanggal'])) ?> WIB</div>
                </td>
                ...
                <td class="id-comp-cell">
                  <?php if ($isSent): ?>
                    <span class="id-cell" title="<?= htmlspecialchars($r['id_diet']) ?>"><?= htmlspecialchars($shortId) ?></span>
                    <button class="btn-copy" onclick="copyText('<?= addslashes($r['id_diet']) ?>')" title="Salin"><i class="fa fa-copy"></i></button>
                  <?php else: ?>
                ...
                <td class="text-center">
                  <span class="badge-comp-status <?= $isSent?'badge-sent':'badge-pending' ?>">
          ...
          <div style="padding:50px;text-align:center;">
            <i class="fa fa-leaf" style="font-size:52px;color:#ddd;display:block;margin-bottom:14px;"></i>
            <h4 style="color:#aaa;font-weight:400;">Tidak ada data diet/gizi pada periode <strong><?= $periode_label ?></strong></h4>
            <p style="color:#bbb;font-size:12px;">
              <i class="fa fa-info-circle"></i>
              Data muncul jika kolom <strong>instruksi</strong> di tabel <strong>catatan_adime_gizi</strong> terisi.
            </p>
          </div>
          <?php endif; ?>
        </div>
...
<div id="modal-log-overlay" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,.55);z-index:9990;align-items:center;justify-content:center;">
  <div style="background:#1e1e1e;border-radius:8px;width:860px;max-width:95vw;max-height:85vh;display:flex;flex-direction:column;box-shadow:0 8px 40px rgba(0,0,0,.5);">
    <div style="background:#2d2d2d;padding:12px 18px;border-radius:8px 8px 0 0;display:flex;align-items:center;justify-content:space-between;border-bottom:1px solid #444;">
      <span style="color:#27ae60;font-weight:700;font-size:14px;font-family:'Courier New',monospace;">
        <i class="fa fa-file-text-o"></i> Log Composition — logs/composition_<?= date('Y-m') ?>.log
      </span>
      <div style="display:flex;gap:8px;">
        <button onclick="refreshLog()" style="background:#27ae60;border:none;color:#fff;padding:4px 12px;border-radius:4px;font-size:12px;cursor:pointer;"><i class="fa fa-refresh"></i> Refresh</button>
...
function refreshLog(){
    ...
    fetch('?_log=composition&_t='+Date.now()).then(r=>r.text()).then(txt=>{_rawLog=txt;renderLog(txt);})
