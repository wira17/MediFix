...
function postServiceRequest(array $row): string {
    if (empty($row['ihs_number']))  throw new RuntimeException('ihs_number pasien kosong — sinkronkan data pasien ke Satu Sehat terlebih dahulu');
    if (empty($row['id_encounter'])) throw new RuntimeException('id_encounter kosong — buat Encounter terlebih dahulu');
    if (empty($row['ihs_dokter']))   throw new RuntimeException('ihs_dokter kosong — lengkapi data dokter Satu Sehat');
