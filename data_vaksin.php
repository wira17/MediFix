...
try {
    ...
    if ($filter_status === 'terkirim') {
        $statusWhere = " AND si.id_immunization IS NOT NULL AND si.id_immunization != ''";
    } elseif ($filter_status === 'pending') {
        $statusWhere = " AND (si.id_immunization IS NULL OR si.id_immunization = '')";
    }
    ...
    // LEFT JOIN encounter agar semua vaksin yang ada mapping tetap muncul
    $baseSQL = "
        ...
        LEFT JOIN satu_sehat_encounter se    ON se.no_rawat        = dpo.no_rawat
    ...
    $stmtData = $pdo_simrs->prepare("
        SELECT
            ...
            IFNULL(si.id_immunization,'') AS id_immunization
        $baseSQL
    ...
    $stmtStats = $pdo_simrs->prepare("
        SELECT
            COUNT(*) AS total,
            SUM(CASE WHEN si.id_immunization IS NOT NULL AND si.id_immunization != '' THEN 1 ELSE 0 END) AS terkirim,
            SUM(CASE WHEN si.id_immunization IS NULL OR si.id_immunization = '' THEN 1 ELSE 0 END) AS pending
        FROM detail_pemberian_obat dpo
        JOIN satu_sehat_mapping_vaksin mv    ON mv.kode_brng      = dpo.kode_brng
        LEFT JOIN satu_sehat_encounter se    ON se.no_rawat       = dpo.no_rawat
        LEFT JOIN satu_sehat_immunization si ON si.no_rawat       = dpo.no_rawat
...
.btn-send:hover{background:#6a1b9a;transform:scale(1.12);box-shadow:0 3px 10px rgba(123,31,162,.45)}
.btn-send:disabled{opacity:.4;cursor:not-allowed;transform:none;background:#ccc;border-color:#bbb}
...
function syncIHS(noRkm,btnEl) {
    btnEl.disabled=true;const orig=btnEl.innerHTML;
    ...
    .then(r=>r.json()).then(resp=>{
        if(resp.status==='ok'){
            btnEl.closest('.ihs-wrap').innerHTML=`<span class="ihs-ok"><i class="fa fa-id-card"></i> IHS OK</span>`;
            showToast('IHS: '+resp.ihs_number,'success');
        } else { btnEl.disabled=false;btnEl.innerHTML=orig;showToast('Gagal sync IHS: '+(resp.message||''),'error'); }
...
function kirimSatu(noRawat,kodeBrng,tglPemberian,jam,btnEl) {
    ...
    fetch(window.location.href,{method:"POST",headers:{"Content-Type":"application/x-www-form-urlencoded"},
        body:new URLSearchParams({action:"kirim_vaksin",no_rawat:noRawat,kode_brng:kodeBrng,tgl_pemberian:tglPemberian,jam:jam})})
    .then(r=>r.json()).then(resp=>{
        ...
        if(resp.status==="ok"){
            ...
            if(row){
                ...
                if(badge){badge.className="badge-vaksin-status badge-sent";badge.innerHTML=`<i class="fa fa-check-circle"></i> Terkirim`;}
                ...
                if(idCell&&resp.id_imunisasi){
                    ...
                    idCell.innerHTML=`<span class="id-cell" title="${resp.id_imunisasi}">${short}</span>
                        <button class="btn-copy" onclick="copyText('${resp.id_imunisasi}')"><i class="fa fa-copy"></i></button>`;
                }
            }
...
<div class="content-wrapper">
  ...
  <section class="content">
    <?php if ($dbError): ?>
    <div class="callout callout-danger"><h4><i class="fa fa-ban"></i> Error Database</h4><p><?= htmlspecialchars($dbError) ?></p></div>
    <?php endif; ?>
    ...
    <div class="row"><div class="col-xs-12">
        ...
        <div class="box-body">
          <form method="GET" class="form-inline" style="display:flex;flex-wrap:wrap;gap:10px;align-items:flex-end;">
            ...
            <div class="form-group">
              ...
              <input type="text" name="cari" class="form-control" placeholder="Pasien / No. Rawat / Nama Vaksin…"
      ...
      <div class="box box-primary" style="border-top-color:#7b1fa2;">
        ...
        <div class="info-bar-stats">
          <div class="ibs-item"><i class="fa fa-database" style="color:#7b1fa2;"></i> Total: <span class="ibs-val" style="color:#7b1fa2;"><?= number_format($st_total) ?></span></div>
          ...
          <span class="ibs-sep">|</span>
          <div class="ibs-item" style="font-size:11px;color:#777;"><i class="fa fa-info-circle"></i> Data dari <strong>detail_pemberian_obat</strong> yang ada mapping KFA vaksin &amp; sudah ada Encounter</div>
        </div>
        ...
        <div class="box-body" style="padding:0;">
            ...
            <table class="table table-bordered table-striped table-hover tbl-vaksin" style="margin-bottom:0;font-size:12px;">
              <thead>
                <tr>
                  ...
                  <th width="180">Vaksin</th>
                  <th width="100">Lot / Batch</th>
                  <th width="85"  class="text-center">Expired</th>
                  <th width="60"  class="text-center">Dosis</th>
                  <th width="70"  class="text-center">Jenis</th>
                  <th width="100">Tgl Pemberian</th>
                  <th width="120">Dokter / Poli</th>
                  <th>ID Immunization</th>
                  <th width="110" class="text-center">Status</th>
                </tr>
              </thead>
              <tbody>
              <?php $no = $offset + 1; foreach ($data as $r):
                  $isSent      = !empty($r['id_immunization']);
                  $hasEncounter= !empty($r['id_encounter']);
                  $shortId     = $isSent ? mb_strimwidth($r['id_immunization'], 0, 36, '…') : '';
              ...
              <tr class="<?= $isSent ? 'row-sent' : '' ?>">
                ...
                <td class="text-center">
                  <button class="btn-send <?= $isSent ? 'sent' : '' ?>"
                          onclick="kirimSatu('<?= addslashes($r['no_rawat']) ?>','<?= addslashes($r['kode_brng']) ?>','<?= htmlspecialchars($r['tgl_perawatan']) ?>','<?= htmlspecialchars($r['jam']) ?>',this)"
                ...
                <td>
                  ...
                  <div class="rm-lbl"><?= htmlspecialchars($r['no_ktp'] ?: '-') ?><?= $umur?" · $umur":'' ?></div>
                ...
                <td class="text-center">
                  <?php if (!empty($r['kadaluarsa'])): ?>
                    <span class="<?= $isExpired?'exp-warn':'exp-ok' ?>">
                ...
                <td class="text-center">
                  <strong><?= number_format((float)$r['jml'], 1) ?></strong>
                ...
                <td>
                  ...
                  <div style="font-size:10.5px;color:#aaa;"><?= substr($r['jam'],0,5) ?> WIB</div>
                </td>
                ...
                <td class="id-vaksin-cell">
                  <?php if ($isSent): ?>
                    <span class="id-cell" title="<?= htmlspecialchars($r['id_immunization']) ?>"><?= htmlspecialchars($shortId) ?></span>
                    <button class="btn-copy" onclick="copyText('<?= addslashes($r['id_immunization']) ?>')" title="Salin"><i class="fa fa-copy"></i></button>
                  <?php else: ?>
          ...
          <div style="padding:50px;text-align:center;">
            ...
            <p style="color:#bbb;font-size:12px;">
              ...
              Data muncul jika ada pemberian obat di <strong>detail_pemberian_obat</strong> yang kode barangnya
              sudah dimapping di <strong>satu_sehat_mapping_vaksin</strong>.
            </p>
          </div>
          <?php endif; ?>
        </div>
