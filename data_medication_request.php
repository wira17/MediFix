<?php
/**
 * data_medication_request.php
 * Monitoring MedicationRequest Satu Sehat
 * Tabel: satu_sehat_medicationrequest + resep_obat + databarang + reg_periksa + dokter
 * Filter: tanggal dari - sampai (berdasarkan tgl_perawatan di resep_obat)
 * Tampilan saja — tanpa fitur kirim (sama seperti medication dispense)
 */
...
// ── Ambil Data ───────────────────────────────────────────────────
try {
    $wheres = ["ro.tgl_perawatan BETWEEN ? AND ?"];
    $params = [$tgl_dari, $tgl_sampai];

    if (!empty($cari)) {
        $wheres[] = "(mr.no_resep LIKE ? OR mr.kode_brng LIKE ? OR mr.id_medicationrequest LIKE ?
    ...
    if ($filter_status === 'terkirim') {
        $wheres[] = "(mr.id_medicationrequest IS NOT NULL AND mr.id_medicationrequest != '')";
    } elseif ($filter_status === 'pending') {
        $wheres[] = "(mr.id_medicationrequest IS NULL OR mr.id_medicationrequest = '')";
    }
    ...
    $base_join = "
        FROM satu_sehat_medicationrequest mr
        JOIN resep_obat ro           ON mr.no_resep     = ro.no_resep
        ...
        LEFT JOIN databarang db      ON mr.kode_brng    = db.kode_brng
    ...
    $stmtStats = $pdo_simrs->prepare("
        SELECT
            COUNT(*) AS total,
            SUM(CASE WHEN mr.id_medicationrequest IS NOT NULL AND mr.id_medicationrequest != '' THEN 1 ELSE 0 END) AS terkirim,
            SUM(CASE WHEN mr.id_medicationrequest IS NULL OR mr.id_medicationrequest = '' THEN 1 ELSE 0 END) AS pending,
...
$page_title = 'Medication Request — Satu Sehat';
...
$extra_css = '
:root { --req-primary:#e65100; --req-dark:#bf360c; --req-soft:#fff3e0; --req-border:#ffe0b2; }
...
.norawat-lbl{font-weight:700;color:var(--req-primary);font-size:12px;font-family:"Courier New",monospace}
.resep-lbl{font-weight:700;color:var(--req-primary);font-size:13px;font-family:"Courier New",monospace}
.rm-lbl{font-size:10.5px;color:#aaa;font-family:"Courier New",monospace}
.nm-pasien{font-weight:700;color:#333;font-size:13px}
.kode-lbl{display:inline-block;background:var(--req-soft);border:1px solid var(--req-border);color:var(--req-primary);padding:2px 7px;border-radius:4px;font-size:11px;font-weight:700;font-family:"Courier New",monospace}
...
.tgl-step{font-size:10.5px;color:#888;display:flex;align-items:center;gap:4px;margin-top:2px}

.info-bar{display:flex;gap:16px;padding:8px 15px;background:#fff8f0;border-bottom:1px solid var(--req-border);font-size:12px;flex-wrap:wrap;align-items:center}
...
.prog-fill{height:100%;border-radius:4px;background:linear-gradient(90deg,var(--req-primary),#00a65a);transition:width .6s}
.prog-pct{width:38px;text-align:right;font-weight:700;color:#555}

.tbl-req thead tr th{background:var(--req-primary);color:#fff!important;white-space:nowrap;font-size:12px}
.tbl-req tbody td{vertical-align:middle}
...
.btn-period:hover{background:var(--req-primary);color:#fff;border-color:var(--req-primary)}
.info-readonly{background:var(--req-soft);border-left:4px solid var(--req-primary);padding:8px 14px;border-radius:4px;font-size:12.5px;color:var(--req-dark)}
';
...
<div class="content-wrapper">
  <section class="content-header">
    <h1>
      <i class="fa fa-file-medical" style="color:#e65100;"></i>
      Medication Request
    ...
    <ol class="breadcrumb">
      ...
      <li>Satu Sehat</li>
      <li class="active">Medication Request</li>
    </ol>
  </section>
  ...
  <section class="content">
    ...
    <div class="row">
      <div class="col-xs-12">
          ...
          <div class="box-body">
            <form method="GET" style="display:flex;flex-wrap:wrap;gap:10px;align-items:flex-end;">
              ...
              <div class="form-group">
                ...
                <input type="text" name="cari" class="form-control" placeholder="Pasien / Dokter / No.Resep / Obat…" value="<?= htmlspecialchars($cari) ?>" style="width:220px;">
              </div>
              <div class="form-group">
              ...
              <div class="form-group" style="align-self:flex-end;">
                ...
                <a href="data_medication_request.php" class="btn btn-default"><i class="fa fa-refresh"></i> Reset</a>
              </div>
            </form>
          </div>
        </div>
        ...
        <!-- Tabel -->
        <div class="box" style="border-top:3px solid var(--req-primary);">
          <div class="box-header with-border">
            <h3 class="box-title" style="color:var(--req-primary);">
              <i class="fa fa-table"></i> Data Medication Request
              <span class="badge" style="background:var(--req-primary);"><?= number_format($total) ?></span>
            </h3>
            <div class="box-tools pull-right">
          ...
          <!-- Info readonly -->
          <div style="padding:8px 15px;border-bottom:1px solid #ffe0cc;">
            <div class="info-readonly">
              <i class="fa fa-info-circle"></i>
              Halaman ini hanya menampilkan data. Pengiriman Medication Request ke Satu Sehat dilakukan otomatis oleh sistem Khanza.
            </div>
          </div>

          <!-- Stats bar -->
          <div class="info-bar">
            <div class="ibs-item"><i class="fa fa-database" style="color:var(--req-primary);"></i> Total: <span class="ibs-val" style="color:var(--req-primary);"><?= number_format($st_total) ?></span></div>
          ...
          <!-- Progress -->
          <div class="prog-row">
            <span style="width:130px;font-size:12px;color:#555;flex-shrink:0;"><i class="fa fa-file-text-o"></i> Med. Request</span>
          ...
          <div class="box-body" style="padding:0;">
            ...
            <div class="table-responsive">
              <table class="table table-bordered table-striped table-hover tbl-req" style="margin-bottom:0;font-size:12.5px;">
                <thead>
                  <tr>
                    ...
                    <th width="150">Tgl Perawatan</th>
                    <th width="150">Tgl Peresepan</th>
                ...
                <tbody>
                  ...
                  $no = $offset + 1;
                  foreach ($data as $r):
                      $isSent  = !empty($r['id_medicationrequest']);
                      $shortId = $isSent ? mb_strimwidth($r['id_medicationrequest'], 0, 36, '…') : '';
                  ...
                  <tr class="<?= $isSent ? 'row-sent' : '' ?>">
                    ...
                    <td>
                      ...
                      <div class="norawat-lbl" style="margin-top:2px;"><?= htmlspecialchars($r['no_rawat']) ?></div>
                    </td>
                    ...
                    <td>
                      ...
                      <?php if (!empty($r['kd_sps'])): ?>
                      <div style="font-size:10px;color:#aaa;">Sp: <?= htmlspecialchars($r['kd_sps']) ?></div>
                      <?php endif; ?>
                    </td>
                    ...
                    <td>
                      ...
                      <div class="tgl-step" style="color:#00a65a;">
                        <i class="fa fa-check"></i>
                        Serah: <?= date('d/m H:i', strtotime($r['tgl_penyerahan'].' '.$r['jam_penyerahan'])) ?>
                      </div>
                      <?php endif; ?>
                    </td>

                    <td>
                      <?php if (!empty($r['tgl_peresepan']) && $r['tgl_peresepan'] !== '0000-00-00'): ?>
                      <div class="waktu-lbl"><?= date('d/m/Y', strtotime($r['tgl_peresepan'])) ?></div>
                      <div class="waktu-sub"><?= !empty($r['jam_peresepan']) ? date('H:i', strtotime($r['jam_peresepan'])).' WIB' : '-' ?></div>
                      <?php else: ?>
                    ...
                    <td>
                      <?php if ($isSent): ?>
                        <span class="id-cell" title="<?= htmlspecialchars($r['id_medicationrequest']) ?>"><?= htmlspecialchars($shortId) ?></span>
                        <button class="btn-copy" onclick="copyText('<?= addslashes($r['id_medicationrequest']) ?>')" title="Salin ID"><i class="fa fa-copy"></i></button>
                      <?php else: ?>
            ...
            <div style="padding:50px;text-align:center;">
              <i class="fa fa-file-text-o" style="font-size:52px;color:#ddd;display:block;margin-bottom:14px;"></i>
              <h4 style="color:#aaa;font-weight:400;">
                <?= ($cari||$filter_status||$filter_lanjut)
                    ? "Tidak ada data untuk filter yang dipilih"
                    : "Tidak ada Medication Request pada periode <strong>$periode_label</strong>" ?>
              </h4>
